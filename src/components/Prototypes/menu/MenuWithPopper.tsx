import { usePopper } from "@/utils/popperjs";
...
export default defineComponent({
  props: {
    ...
    itemsClass: {
      type: String,
      default:
        "absolute right-0 w-56 origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none",
    },
    itemClass: {
      type: Object,
      default: () => ({
            ...
            {() => (
              <>
                <span class={props.buttonClass.container}>
                  <MenuButton ref={trigger} class={props.buttonClass.default}>
                    {() =>
                      slots.button ? (
                        slots.button()
                      ) : (
                ...
                <Teleport to="#app">
                  <MenuItems ref={container} class={props.itemsClass}>
                    {() =>
                      slots.items ? (
                        slots.items()
                      ) : (
                        <>
                          ...
                          <div class="py-1">{menuItems.map((item) => renderMenuItem(item))}</div>
